      ...
      try {
        ...
        if (itemDoc.exists()) {
          ...
          if (itemData.isAuction && user?.uid === itemData.ownerId) {
            ...
            // OPTIMIZATION: Prevent N+1 queries by extracting distinct IDs and using batched concurrent requests
            ...
            const chunkSize = 30;
            const itemChunks = [];
            for (let i = 0; i < uniqueItemIds.length; i += chunkSize) itemChunks.push(uniqueItemIds.slice(i, i + chunkSize));

            const userChunks = [];
            for (let i = 0; i < uniqueUserIds.length; i += chunkSize) userChunks.push(uniqueUserIds.slice(i, i + chunkSize));
            ...
            await Promise.all([
              ...itemChunks.map(async (chunk) => {
                ...
                itemsSnap.docs.forEach(doc => { itemsDict[doc.id] = doc.data(); });
              }),
              ...userChunks.map(async (chunk) => {
                if (chunk.length === 0) return;
                const usersQuery = query(collection(db, 'users'), where(documentId(), 'in', chunk));
                const usersSnap = await getDocs(usersQuery);
                usersSnap.docs.forEach(doc => { usersDict[doc.id] = doc.data(); });
              })
            ]);
