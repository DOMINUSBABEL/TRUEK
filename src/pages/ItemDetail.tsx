      ...
      try {
        ...
        if (itemDoc.exists()) {
          ...
          if (itemData.isAuction && user?.uid === itemData.ownerId) {
            ...
            // OPTIMIZATION: Prevent N+1 queries by extracting distinct item IDs and user IDs
            ...
            // OPTIMIZATION: Firestore limits 'in' queries to 30 elements, chunk the requests
            const chunkSize = 30;
            const itemChunks = [];
            ...
            const itemPromises = itemChunks.map(async (chunk) => {
            ...
            await Promise.all([...itemPromises, ...userPromises]);
