      ...
      try {
        ...
        if (itemDoc.exists()) {
          ...
          if (itemData.isAuction && user?.uid === itemData.ownerId) {
            ...
            // OPTIMIZATION: Extract distinct IDs to prevent N+1 queries
            ...
            // OPTIMIZATION: Fetch all documents concurrently in chunks
            await Promise.all([
              ...itemChunks.map(async (chunk) => {
                if (chunk.length === 0) return;
                const itemsQuery = query(collection(db, 'items'), where(documentId(), 'in', chunk));
                const itemsSnapshot = await getDocs(itemsQuery);
                itemsSnapshot.docs.forEach(d => { itemsDict[d.id] = d.data(); });
              }),
              ...userChunks.map(async (chunk) => {
                if (chunk.length === 0) return;
                const usersQuery = query(collection(db, 'users'), where(documentId(), 'in', chunk));
                const usersSnapshot = await getDocs(usersQuery);
                usersSnapshot.docs.forEach(d => { usersDict[d.id] = d.data(); });
              })
            ]);
