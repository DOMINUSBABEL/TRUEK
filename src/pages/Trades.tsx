...
import { ArrowRightLeft, MessageCircle, CheckCircle, XCircle, Search, Inbox, Loader2, X } from 'lucide-react';
...
export default function Trades() {
  ...
  const fetchTrades = async () => {
    ...
    try {
      ...
      // OPTIMIZATION: Prevent N+1 queries by extracting distinct item IDs
      // and fetching them concurrently in bulk rather than per-trade.
      ...
      // OPTIMIZATION: Firestore limits 'in' queries to 30 elements, so chunk the requests
      const chunkSize = 30;
      const chunks = [];
      for (let i = 0; i < uniqueItemIds.length; i += chunkSize) {
        chunks.push(uniqueItemIds.slice(i, i + chunkSize));
      }
      ...
      await Promise.all(chunks.map(async (chunk) => {
        ...
        itemsSnapshot.docs.forEach(doc => {
          itemsDict[doc.id] = doc.data(); // Build O(1) local cache dictionary
        });
      }));
      ...
      const tradesWithDetails = snapshot.docs.map(tradeDoc => {
        ...
        return {
          id: tradeDoc.id,
          ...data,
          targetItem: itemsDict[data.targetItemId] || null,
          offeredItem: itemsDict[data.offeredItemId] || null
        };
      });
    ...
    <div className="p-6 pb-32 bg-neutral min-h-screen">
      ...
      <div className="mb-8 relative">
        ...
        <input
          ...
          className="w-full pl-11 pr-10 py-3.5 bg-surface border border-white/10 rounded-2xl text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-primary/50 focus:border-primary/50 transition-all shadow-sm"
        />
        {searchQuery && (
          <button
            type="button"
            ...
            className="absolute right-3 top-1/2 -translate-y-1/2 p-1.5 text-gray-400 hover:text-white transition-colors focus-visible:ring-2 focus-visible:ring-primary focus-visible:outline-none rounded-full"
          >
