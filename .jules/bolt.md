...
## 2026-06-04 - Use writeBatch for atomic updates
**Learning:** Using `writeBatch` instead of sequential `updateDoc` calls reduces network latency from O(N) to O(1) for multi-document operations and ensures atomicity.
**Action:** Always consider `writeBatch` when updating multiple related documents simultaneously.
