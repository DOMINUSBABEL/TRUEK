...
## 2024-06-25 - Firestore Write Batch Optimization
**Learning:** Sequential `updateDoc` operations on related entities (like updating a trade, followed by two item statuses) create an N+1 write bottleneck and potential data inconsistency in Firestore.
**Action:** Always prefer `writeBatch` when updating multiple documents simultaneously in response to a single user action (like accepting a trade).
